...
export class MemberSignupFormPage implements OnInit {
  ...
  constructor(
    ...
    async onChange(event) {
      ...
      this.logoIMG = firebase.storage().ref('temp-businesses-logos/' + this.files[0].name);
    ...
    async createAnnouncement(announcementForm): Promise<void> {
      ...
      console.log(personFirstName, personLastName, personBirthday, personEmail, personNIFCIN, companyStatus, companyName, companyDescription, categoryID, companyEnrollDate, companyEmail, whatsappNumber, phoneNumber, companyPhone3, companyAddress, companyState, companyCity, companyWebsite, self.logoImgURL, this.fileName);
      
      self.annoucementService
      .createTempAnnouncement(personFirstName, personLastName, personBirthday, personEmail, personNIFCIN, companyStatus, companyName, companyDescription, categoryID, companyEnrollDate, companyEmail, whatsappNumber, phoneNumber, companyPhone3, companyAddress, companyState, companyCity, companyWebsite, self.logoImgURL, this.fileName)
      .then(() => {
        ...
        self.loading.dismiss().then(async () => {
          const alert = await self.alertCtrl.create({
            ...
            message: "Votre inscription a été enregistré. Notre equipe vous contactera sous peu pour les suivies nécessaires!",
            buttons: [{
              text: "Merci",
              handler: () => {
